...
while running:
    ...
    for name in inters:
        ...
        print(f'Cycle {cycles}: {len(results)} results, {len(inters)} to process, {len(done)} exhausted - on index {inters.index(name)}')

        # name = inters[i]
        out = []
        if name[-1] not in rules.keys():
            done.append(name)
            del inters[inters.index(name)]
            continue
        for l in rules[name[-1]]:
            out.append(name + l)
        for o in out:
        ...
        # assert all(x not in inters for x in done)

         #, {len(out)} processed this run')
